...
async function main() {
  ...
  // Cek token GitHub tidak masih placeholder / terlalu pendek
  if (
    !CONFIG.GITHUB_TOKEN ||
    CONFIG.GITHUB_TOKEN.includes("PASTE_TOKEN") ||
    CONFIG.GITHUB_TOKEN.length < 20
  ) {
    throw new Error(
      "GITHUB_TOKEN di config.js masih placeholder / invalid.\n" +
      "Generate token baru di https://github.com/settings/tokens\n" +
      "(Classic: centang repo + workflow) lalu paste ke config.js"
    );
  }
  ...
  console.log(`🐙 GitHub   : ${CONFIG.GITHUB_USERNAME}/${CONFIG.GITHUB_REPO}`);
