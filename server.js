...
function githubRequest(method, path, body = null, extraHeaders = {}) {
  return new Promise((resolve, reject) => {
    if (!config.GITHUB_TOKEN || config.GITHUB_TOKEN.includes("PASTE_TOKEN") || config.GITHUB_TOKEN.length < 20) {
      return reject(new Error("GITHUB_TOKEN belum diisi / masih placeholder. Buka config.js dan isi token GitHub yang valid."));
    }
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        ...
        // Beri pesan jelas kalau token jelek
        if (res.statusCode === 401) {
          return reject(new Error(
            "GitHub token INVALID / expired (401 Bad credentials). " +
            "Generate token baru di https://github.com/settings/tokens " +
            "(scope: repo + workflow) lalu update di config.js"
          ));
        }
...
async function createRelease(tagName, isDraft = true) {
  ...
  if (res.status !== 201) {
    ...
    throw new Error(`Gagal buat release (HTTP ${res.status}): ${msg}`);
  }
...
async function uploadZipToRelease(filePath, fileName, tagName) {
  ...
  // Gunakan API asset URL (bukan browser_download_url) supaya workflow
  // bisa download draft release dengan Authorization header.
  return {
    releaseId: release.releaseId,
    assetId: upload.body.id,
    assetUrl: upload.body.url,
    browserUrl: upload.body.url, // ini API URL, dipakai sebagai zip_url di workflow
  };
}
